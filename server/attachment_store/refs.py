...
bearer token). This prevents a lower-privilege token from reading inline
image refs that were cached by a higher-privilege token during the same TTL
window. The raw token is never stored; only its digest is persisted.
"""
...
def _migrate_add_token_hash(backend) -> None:
    ...
    # Rebuild with new schema; existing rows are dropped because the old
    # primary key is incompatible and backfilling a meaningful token hash
    # is not possible.
...
def write_inline_image_refs(
    ...
    *,
    ...
    The token is hashed with SHA-256 before storage; the raw value is never
    ...
    Args:
        ...
        token:     Raw bearer token of the caller.
    """
...
def read_inline_image_refs(
    ...
    *,
    ...
    Only rows written by the same token (matched via SHA-256 digest) are
    returned, preventing cross-token ref leakage.
