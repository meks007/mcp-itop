...
def register(mcp, client: ItopClient):
    ...
    async def itop_search_kb(
        ...
        KB article class: ITOP_KB_CLASS (configured in .env, default KBEntry).

        The ID column in the result contains confirmed integer database IDs.
        Pass them directly to Get_KB_article or Load_object with
        obj_class=ITOP_KB_CLASS without calling Resolve_object first.
