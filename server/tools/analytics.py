...
def register(mcp, client: ItopClient):
    ...
    async def itop_sla_report(
        ...
        """Report TTO/TTR pass, breach, and N/A rates plus resolution-time statistics for a date range, optionally filtered by service.
        """
    ...
    async def itop_agent_workload(
        ...
        """Summarize ticket counts, open/closed volume, time spent, and backlog by agent. Filter by agent or team; agents with more than 10 open tickets are flagged as overloaded.
        """
        s, e = parse_date_range(start_date, end_date)
    ...
    async def itop_idle_agents(
        ...
        """Find tickets idle longer than hours (default 2) in the selected status (default assigned). A ticket is idle only when both assignment_date and last_update are older than the threshold. Returns one row per ticket.
        """
    ...
    async def itop_service_quality(
        ...
        """Find recent groups of similar tickets assigned to different services. Titles are grouped when they share at least two non-stop words of three or more characters; groups smaller than min_similar are ignored. days defaults to 30.
        """
        import re
    ...
    async def itop_caller_quality(
        ...
        """Rank callers by how consistently tickets use one service over the last N days. Includes callers with at least min_tickets. This distribution heuristic does not verify whether a service selection was correct.
        """
        s = (datetime.now(timezone.utc) - timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S")
        e = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    ...
    async def itop_agent_correction_rate(
        ...
        """Measure service diversity per agent as unique services divided by total tickets. A high score indicates broad service coverage or recategorisation, not whether the agent corrected a caller's selection.
        """
        s = (datetime.now(timezone.utc) - timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S")
        e = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    ...
    async def itop_ticket_summary(
        ...
        """Summarize ticket volume, status, SLA breaches, and resolution times for the last N days (default 30).
        """
        s = (datetime.now(timezone.utc) - timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S")
        e = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
